...
const RankingButton = () => {
  ...
  return (
    <Popover className="relative">
      {({ open }) => (
        <>
          <Popover.Button className="flex items-center gap-2 px-4 py-2 rounded-xl border border-yellow-500/50 bg-black/40 text-yellow-300 text-sm font-semibold hover:bg-yellow-500/10 hover:border-yellow-400 transition-all duration-200 shadow-md focus:outline-none">
          ...
          >
            <Popover.Panel className="absolute left-0 z-20 mt-2 w-112">
              <div className="rounded-xl border border-yellow-500/30 bg-gray-900/95 backdrop-blur-sm shadow-xl overflow-hidden">
                ...
                {loading ? (
                  ...
                  <p className="text-center text-xs text-gray-500 py-4">Sin datos aún</p>
                ) : (
                  <ul className="py-1">
                    {ranking.map((entry, i) => (
                      <li
                        key={entry.username}
                        className="flex items-center gap-2 px-3 py-1.5 hover:bg-yellow-500/5 transition-colors"
                      >
